...
def send_email(subject, message):
    ...
    params = {
        'Destination': {
            'ToAddresses': ['hectorlopezluis@hotmail.com']
        },
        'Message': {
            'Body': {
                'Text': {
        ...
        'Source': 'hectorlopezluis@hotmail.com'
    }

    try:
        response = ses.send_email(**params)
        print(f"Email enviado correctamente. MessageId: {response['MessageId']}")
        return response
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if not df.empty:
            ...
            # Inserción masiva con bulkWrite y upsert
            def insertar_por_bulk(df, collection, chunk_size=10000):
                for start in range(0, len(df), chunk_size):
                    end = start + chunk_size
                    subset = df.iloc[start:end].to_dict("records")

                    operaciones = [
                        UpdateOne({"numRegistro": row["numRegistro"]}, {"$setOnInsert": row}, upsert=True)
                        for row in subset
                    ]

                    if operaciones:
                        result = collection.bulk_write(operaciones, ordered=False)
                        print(f"Chunk {start}-{end}: Total registros insertados {result.upserted_count}")
                        send_email('TFM - Carga de nuevos datos', f'Total registros insertados {result.upserted_count}')

            # Ejecutar inserción por chunks
            insertar_por_bulk(df_final, collection)

        else:
            print("Todos los registros estaban duplicados")
            send_email('TFM - Carga de nuevos datos', 'Todos los registros estaban duplicados')
        ...
        print(f"Error: {e}")
        send_email('TFM - Carga de nuevos datos [ERROR]', 'Se produjo un error en la subida del fichero')
        return {
            'statusCode': 500,
            'body': f"Error: {str(e)}"
        }